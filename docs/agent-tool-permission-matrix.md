...
Inventory allowed tools, denied tools, escalation rules, and data exposure boundaries.

## When To Use

Use before enabling shell, browser, MCP, deployment, database, or third-party integration tools.
...
Scenario: A reviewer needs to decide whether an agent task using Tool Permission Matrix can continue.

Completed example: Fill docs/agent-tool-permission-matrix.md with task scope, evidence, owner, approval decision, and rollback path before the agent proceeds.
