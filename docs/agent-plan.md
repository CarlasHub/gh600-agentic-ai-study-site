...
Require a plan before edits, including assumptions, files, validation, risks, and decision points.

## When To Use

Use before allowing an agent to make non-trivial or cross-file changes.
...
Scenario: A reviewer needs to decide whether an agent task using Agent Plan can continue.

Completed example: Fill docs/agent-plan.md with task scope, evidence, owner, approval decision, and rollback path before the agent proceeds.
